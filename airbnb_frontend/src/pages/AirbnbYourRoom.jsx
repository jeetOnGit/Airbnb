  ...
  useEffect(() => {
    const fetchImage = async () => {
      try {
        ...
        const imageDataArray1 = bannerData1.data.banners[0].image.data.data;
        const uint8Array1 = new Uint8Array(imageDataArray1);
        const blob1 = new Blob([uint8Array1], { type: bannerData1.data.banners[0].image.contentType });
        const imageUrl1 = URL.createObjectURL(blob1);
        setImageSrc1(imageUrl1);


        const bannerData2 = await response.json();
        const imageDataArray2 = bannerData2.data.banners[1].image.data.data;
        const uint8Array2 = new Uint8Array(imageDataArray2);
        const blob2 = new Blob([uint8Array2], { type: bannerData2.data.banners[1].image.contentType });
    ...
    <>
          ...
          <div className="mapSection py-12 flex justify-between items-center max-[1200px]:flex-col max-[1200px]:gap-9">
            <div className="hotelValue text-center mx-auto w-[50%] max-[1200px]:w-full">
              ...
              <div className="rateSlider my-5 leading-8">
                <input className='bg-black rangeSlider appearance-none cursor-pointer relative overflow-visible max-[1200px]:w-[90%]' type="range" name="nightSlider" id="" min='1' max='30' value={nights} onChange={handleNightChange} style={{
                  background: `linear-gradient(to right, #222 ${((nights - 1) / 29) * 100}%, #ddd ${((nights - 1) / 29) * 100}%)`
                }} />
      ...
      <section className="faq bg-[#f7f7f7] py-4">
        <div className="myContainer mx-auto">
          <div className="faqContent flex py-12">
            <div className='w-[50%]'>
              <h3 className='text-4xl font-semibold text-[#222]'>Your questions, <br /> answered</h3>
            </div>
            <div className='w-[50%]'>
              <Dropdown />
            </div>
